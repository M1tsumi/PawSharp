...
```
 ...
 `GuildOnly` is currently the only privacy level supported by Discord.
...
```
 ...
 External events **must** have both `ScheduledEndTime` and `EntityMetadataLocation`.
...
```
 ...
 You **cannot** change `EntityType` after creation. You also cannot change the event type from `External` to `Voice` (or vice versa).
