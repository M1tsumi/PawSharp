...
```
6.1.0-alpha-1   ← early development, APIs may change freely
...
main ──┬──────────────────────────────────────────► main
       │
       └── release/v6.1.0-alpha-1 ──► PR ──► merge ──► tag v6.1.0-alpha-1
```
...
chore: bump all packages to 6.1.0-alpha-1
release: v6.1.0-alpha-1
...
git checkout -b release/v6.1.0-alpha-1
...
#    Move content under the new [6.1.0-alpha-1] heading, set the date.

# 4. Commit
git add -A
git commit -m "release: v6.1.0-alpha-1"

# 5. Push the version branch
git push -u origin release/v6.1.0-alpha-1
...
Version format:  6.1.0-alpha-1   (dashes throughout — no dots in pre-release suffix)
Tag format:      v6.1.0-alpha-1  (no spaces, no underscores)
Branch format:   release/v6.1.0-alpha-1
